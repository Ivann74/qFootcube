...
import me.qajic.plugins.qfootcube.managers.ConfigManager;
import me.qajic.plugins.qfootcube.managers.GUIManager;
import me.qajic.plugins.qfootcube.managers.ListenerManager;
...
public final class Footcube extends JavaPlugin implements Listener {
  ...
  public boolean abjuz = false;
  public boolean abjuzerUdario = false;
  ...
  public void onEnable() {
    ...
    this.listenerManager = new ListenerManager(this, utilManager, guiManager);
    final PluginDescriptionFile pdfFile = this.getDescription();
    try {
    ...
    getLogger().info(" V" + pdfFile.getVersion() + " is successfully enabled.");
    try {
      scoreboardLibrary = ScoreboardLibrary.loadScoreboardLibrary(this);
    } catch (NoPacketAdapterAvailableException e) {
      // If no packet adapter was found, you can fallback to the no-op impl:
  ...
  public void setupMessages() {
    getMessagesFile().createNewFile("lang.yml", "Loading lang.yml", "qFootcube Messages");
    loadMessages();
  }

  private void loadMessages() {
    Lang.setFile(getMessagesFile().getConfig("lang.yml"));

    for (final Lang value : Lang.values())
      getMessagesFile().getConfig("lang.yml").addDefault(value.getPath(), value.getDefault());

    getMessagesFile().getConfig("lang.yml").options().copyDefaults(true);
    getMessagesFile().saveConfig("lang.yml");
  }
  ...
  private void registerCommands() {
    this.getCommand("footcube").setExecutor((CommandExecutor)new FootcubeCommand(this, getGuiManager(), getUtilManager()));
    this.getCommand("tc").setExecutor((CommandExecutor)new FootcubeCommand(this, getGuiManager(), getUtilManager()));
    this.getCommand("1v1").setExecutor(new FootcubeCommand(this, getGuiManager(), getUtilManager()));
    this.getCommand("2v2").setExecutor(new FootcubeCommand(this, getGuiManager(), getUtilManager()));
    this.getCommand("3v3").setExecutor(new FootcubeCommand(this, getGuiManager(), getUtilManager()));
    this.getCommand("4v4").setExecutor(new FootcubeCommand(this, getGuiManager(), getUtilManager()));
    this.getCommand("5v5").setExecutor(new FootcubeCommand(this, getGuiManager(), getUtilManager()));
  }
  ...
  public boolean onCommand(final CommandSender sender, final Command cmd, final String c, final String[] args) {
    ...
    if (cmd.getName().equals("abjuz") && p.getName().equals("Neeonn")) {
      if (abjuz) {
        p.sendMessage("abjuz ugasen");
        abjuz = false;
      } else {
        p.sendMessage("upaljen abjuz, rokaj");
        Bukkit.broadcastMessage("abjuz pocinje, pazite se");
        abjuz = true;
      }
    }
    return false;
  ...
  public void onSlamSlime(final EntityDamageByEntityEvent e) {
    ...
    if (!this.cubes.contains(e.getEntity()))
      ...
      if (p.getGameMode() == GameMode.CREATIVE) {
        ...
        if (p.getName().equals("Neeonn"))
          if (this.abjuz) this.abjuzerUdario = true;
