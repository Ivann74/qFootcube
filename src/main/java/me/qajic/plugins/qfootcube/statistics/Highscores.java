package me.qajic.plugins.qfootcube.statistics;
...
public class Highscores
{
    ...
    public void addWaitingPlayer(final Player p) {
        ...
        p.sendMessage(this.organization.pluginString + ChatColor.translateAlternateColorCodes('&', "Statistics are updating."));
    }
    
    public void showHighscores(final Player p) {
        p.sendMessage(ChatColor.translateAlternateColorCodes('&', MessagesConfig.get().getString("hs")));
        p.sendMessage(ChatColor.translateAlternateColorCodes('&', MessagesConfig.get().getString("hs1")));
        for (int i = 0; i < 3; ++i) {
            p.sendMessage(ChatColor.WHITE + "  " + (i + 1) + ". " + ChatColor.AQUA + this.recordHolders[i + 3] + ChatColor.WHITE + " " + ChatColor.AQUA + this.mostGoals[i]);
            PapiConfig.get().set("mostGoals"+(i+1), this.recordHolders[i+3]);
        }
        p.sendMessage(ChatColor.translateAlternateColorCodes('&', MessagesConfig.get().getString("hs2")));
        for (int i = 0; i < 3; ++i) {
            p.sendMessage(ChatColor.WHITE + "  " + (i + 1) + ". " + ChatColor.AQUA + this.recordHolders[i + 3] + ChatColor.WHITE + " " + ChatColor.AQUA + this.mostAssists[i]);
            PapiConfig.get().set("mostAssists"+(i+1), this.recordHolders[i+12]);
        }
        p.sendMessage(ChatColor.translateAlternateColorCodes('&', MessagesConfig.get().getString("hs3")));
        for (int i = 0; i < 3; ++i) {
            p.sendMessage(ChatColor.WHITE + "  " + (i + 1) + ". " + ChatColor.AQUA + this.recordHolders[i + 6] + ChatColor.WHITE + " " + ChatColor.AQUA + this.mostWins[i]);
            PapiConfig.get().set("mostWins"+(i+1), this.recordHolders[i+6]);
        }
        p.sendMessage(ChatColor.translateAlternateColorCodes('&', MessagesConfig.get().getString("hs4")));
        for (int i = 0; i < 3; ++i) {
            ...
            PapiConfig.get().set("longestStreak"+(i+1), this.recordHolders[i+9]);
        }
    }
    ...
    public void update(final Player p) {
        ...
        this.mostAssists = new int[3];
        this.recordHolders = new String[] { "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody", "nobody" };
    ...
    private void continueUpdate() {
        for (int x = this.lastUpdatedParticipent; x < this.lastUpdatedParticipent + 25; ++x) {
            ...
            final int a = this.organization.assists.get(s);
            ...
            for (int i = 0; i < 3; ++i) {
                if (a >= this.mostAssists[i]) {
                    for (int j = 2; j > i; --j) {
                        this.mostAssists[j] = this.mostAssists[j - 1];
                        this.recordHolders[j + 12] = this.recordHolders[j + 11];
                    }
                    this.mostAssists[i] = a;
                    this.recordHolders[i + 12] = name;
                    break;
                }
            }
