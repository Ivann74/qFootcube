    ...
    public void regular(Location location, int type, Footcube pl) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for(int i=0; i<6; i++) {
                    if(type==0) {
                        FallingBlock fallingBlock = location.getWorld().spawnFallingBlock(loc, Material.WOOL, (byte)11);
                        fallingBlock.setVelocity(rotateAroundAxisY(calculateVelocity(loc.toVector(), loc.toVector().add(new Vector(8,0,0)), 6),angle));
                        fallingBlock.setDropItem(false);
                        angle+=45;
                    } else {
                        FallingBlock fallingBlock = location.getWorld().spawnFallingBlock(loc, Material.WOOL, (byte)14);
                        fallingBlock.setVelocity(rotateAroundAxisY(calculateVelocity(loc.toVector(), loc.toVector().add(new Vector(8,0,0)), 6),angle));
                        fallingBlock.setDropItem(false);
                        angle+=45;
                    }
